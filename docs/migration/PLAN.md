...
- `household_member` — `person_id`, `household_id`, role (`student`, `guardian`, `billing_contact`), `relationship` label. An adult student holds two roles rather than occupying two records; the label is imported, not invented — FileMaker already stores `Primary_Relation` / `Secondary_Relation` (findings §2.5).
- `student_profile` — `person_id` primary key, `level_id`, status, `pool` (small, big, either), `big_pool_ready_at` with the confirming instructor. Swim-specific attributes only. Readiness is a dated instructor judgement, never computed: a Level-4 student is surfaced as "assess for big pool" and the checkbox records that they can stand in its shallow end (findings §3.6).
...
- `seat_hold` — `slot_id`, `student_id` (the child it protects), `from`, `review_at` (default four weeks, rule `seat_hold_review_weeks`), reason, `set_by`. A semi-private slot kept free of a second student while a teacher thinks a child needs it; matching never offers the seat, and a prompt fires when the review date passes (findings §3.1, Confirmed).
- `instructor_block` — `instructor_id`, date, time range, kind (`regular_break`, `ad_hoc`), reason. A teacher's break or a day's blocked time, excluded from availability everywhere. Deliberately a second construct: ownership wants the two never to share one flag (findings §3.1). What FileMaker's 9,582 break rows and 42,680 typeless hold rows import as is the schedule-row ticket.
...
- `enrollment_offer` — `slot_id`, `student_id`, `offered_at`, `expires_at` (rule `enrollment_offer_deadline`: 5 pm the following day), `payment_request_id`, status (`open`, `accepted`, `expired`). Holds the seat while the family pays; on expiry the slot returns to availability and the request is cancelled (findings §2.2, Confirmed).
...
- `closure` — date range, scope (all, one pool, one weekday), reason, `issues_credit`. The master calendar Q45 asks about. Declaring an incidental closure lists everyone affected — by weekday or by pool, the two searches staff say make bulk issuance easy — and each student gets a credit unless staff choose the waiver below (package v3 §3.9, Confirmed).
- `closure_waiver` — `closure_id`, `household_id`, `student_id`, kind (`new_student`, `goodwill`), `granted_by`, `at`. Skip the credit and prorate tuition through the proration mechanism instead; the goodwill kind is allowed **once per household** and the row is what makes that rule enforceable across years (package v3 §3.9). Whether "once" is per household or per child is open (qa.md Q114).
- `waitlist_entry` — `student_id`, desired class type, stated availability, `created_at`.
- `makeup_offer` — `credit_id` (reserved for the offer), `lesson_id`, `household_id`, `offered_at`, `expires_at` (rule `makeup_offer_window_hours`, default 2, adjustable per offer), status (`open`, `accepted`, `expired`, `withdrawn`), `next_candidate`. One family at a time per slot; one or two options per family, never a shortlist, because every held slot blocks another family (package v3 §2.4, Confirmed). Expiry advances the slot to the next candidate automatically or on approval per `offer_cascade_auto` (qa.md Q113); the board shows who holds the offer, how long remains, and how many families are waiting on the slot.
...
- `payer_assignment` — `student_id`, `payer_id`, share, date range, plus the student's identity with that agency: UCI number and coordinator contact, structured and restricted to billing/management roles. Siblings on different payers, which Q11 confirmed, is the normal case rather than an exception. Seeded at import from `Students.Payment_Plan`, which is where FileMaker records the charter school (findings §2.14).
...
- `credit_event` — kind (`conversion`, `transfer`, `reversal`, `expiry`, `void`, `reconciliation`), source credit, count consumed, resulting credit, rate applied, actor, `at`; a reconciliation also carries the stored counter, the replayed balance, and who confirmed it. A conversion consumes exactly `from_count` of one denomination and issues exactly `to_count` of another at the table rate, the new credit inheriting the earliest source `valid_to`; a transfer moves credits between siblings in a household, keeping origin and expiry; a reversal negates an earlier event and is management-only. The ledger is append-only. These are the hand-written notes staff keep today (Round 2 Q10), made explicable — settled by [ADR-0003](../adr/0003-denominated-makeup-credit-ledger.md).
- `payment` — `payer_id`, amount, method (`card`, `check`, `direct_deposit`, `cash`, `trade`, …), reference (check number, gateway txn id), `received_at`, `settled_at`, note. No cap on payments per month. A trade — lessons for goods or services — is a payment of method `trade` with its dollar value and a note, so the value is recorded somewhere for the first time (findings §3.3, Confirmed; the accountant's treatment is qa.md Q115).
- `payment_request` — `household_id`, amount (computed from the account, never typed), `created_by`, `approved_by`, channel (`sms`, `email`), hosted link reference, `sent_at`, `expires_at`, status (`draft`, `sent`, `paid`, `cancelled`, `expired`), resulting `payment_id`. The mechanism by which office staff never see or enter a card: the family pays through the link and the outcome posts to the ledger (findings §2.2, ownership decision). Product and flow are map tickets.
- `billing_exemption` — `household_id`, reason (`trade`, `staff_family`, `courtesy`, `other`), note, date range, `granted_by`. Today's "do not bill": about twenty households whose reasons are indistinguishable (findings §3.4, Confirmed). The billing run skips them and a report lists every one with why.
...
- `flag` — subject (student or guardian), kind (`allergy`, `support_need`, `swim_diaper`, `account_handling`), severity, detail, reason. Typed data replacing typography. `account_handling` sits on the guardian, is office-only, carries a short reason, and makes office screens render the guardian's name in caps so the glanceable convention survives (findings §2.1, ownership); the decode pass fills it from the 138 primary and 113 secondary ALL-CAPS contact names.
- `note` — subject, body, audience (`office`, `instructor`, `all`), author, `created_at`. Import reads both the legacy Notes table and `Lesson_Attendance.Intructor_Notes_to_Office`, where instructor-to-office notes also live (findings §2.15).
- `document` — kind (`invoice_pdf`, `intake_form`), storage key, subject. Container fields and the institutional paper trail land here.
- `audit_event` — subject, action, actor, reason, `at`, before/after. Every override action and every deletion writes one. FileMaker's `Audits` table turns out to record only schedule deletions (5,958 rows, findings §2.4) and its `flag_trashcan` soft-delete is set on no row at all, so no general trail exists to import.
...
**Discretion is a design requirement, not a gap** (findings §3.2). Ownership wants to keep using judgement — trades, goodwill credits, refunds versus make-ups, special pricing, a held seat — and the design answer is a short set of general-purpose **override actions**, each requiring who and a one-line reason and each writing an `audit_event`: adjust a bill by an amount (`adjustment`), set a special price (`price_agreement` of source `negotiated`), issue or waive a credit, hold a seat (`seat_hold`), refund or offer a make-up instead. The system does not try to anticipate every case; it makes every case explicable later.
...
| `closure.declared` | Cancel every lesson in range (optionally one pool or weekday), list everyone affected, issue credits where `issues_credit` holds — or record a `closure_waiver` and prorate where staff chose to — send the bulk notice |
...
| `makeup.offered` / `offer.expired` | Hold the slot and reserve the credit for the window; on expiry release both and advance to the next candidate, automatically or on approval |
| `enrollment.offered` / `enrollment_offer.expired` | Hold the seat and create the payment request; at 5 pm the next day unpaid, release the seat and cancel the request |
| `payment_request.paid` | Post the `payment`, apply it to the invoice or offer, confirm the enrollment — no hand matching |
...
Eligibility bars, sibling discount steps, the first-responder percentage, prepay tiers, make-up conversion rates, make-up expiry (`makeup_expiry_months`) and the legacy Christmas redemption boundary (`makeup_redeem_before`, qa.md Q109), closure credit policy and the once-per-household proration waiver, the make-up offer window (`makeup_offer_window_hours`, default 2) and cascade mode (`offer_cascade_auto`), the enrollment-offer deadline (`enrollment_offer_deadline`, 5 pm the following day), the seat-hold review period (`seat_hold_review_weeks`, default 4), the billing-exemption reason list, cancellation cutoffs, and the class-pack rules (which class types sell packs, `adult_no_show_consumes`, `adult_notice_hours`, `class_pack_expiry_months`, `class_pack_discounts_apply`) are rows, not code. Answers to `qa.md` arrive as configuration a staff member can change, and each carries `effective_from`, so changing a rule leaves last year's invoices reproducible.
...
- **Authorize.Net** — kept, with Customer Information Manager holding the card vault so cards survive cutover; Accept.js captures cards in the browser so numbers never touch the server, which ends the plaintext card fields; CIM charges a computed monthly amount (never ARB, whose fixed schedule cannot carry prorations and credits); the processor sits behind one internal interface. Enable Account Updater, which Q39 suggests is off and which turns a failed charge into a silent refresh. Volume is confirmed at $156k–$207k a month (Round 2 Q17), which is why the stack stays. **Payment requests** ride the same gateway: a hosted payment link per household, sent by SMS with the amount computed from the account, so office staff never see a card and no amount is hand-typed (findings §2.2, ownership decision); which Authorize.Net product provides the link, and the flow around it, are map tickets. The legacy card scrub — full numbers replaced with last four, field-level access restricted, backups and export copies cleaned — is legacy operations outside this plan, with one exception: the frozen archive taken at Cutover is made **after** the scrub.
...
The schema from §2 for identity, scheduling, and flags — money tables follow with Billing; the repeatable, non-destructive import against live exports for that slice; the **decode pass** narrowed to every deck-visible convention (ALL-CAPS student names → `support_need` flag, ALL-CAPS guardian first names → `account_handling` flag with a reason, the under-4 diaper rule), with the full decode still gating the wider import; the auth model — individual accounts, PIN at shift start, session and device management; hosting, backup, and point-in-time recovery with one tested restore. An **archive** concept from the start — households restorable, never deleted — because `BlueBuoy_FM_2024` exists only as a performance workaround (families idle ten years moved out of the live file, Round 2 Q18), and what the import does with it is a map decision. The live export of 2026-09-14 ([data profile](../discovery/data-profile-2026-09-14.md)) sized this phase: 13,780 families and 9,443 students in the live file, 856 and 1,163 of them flagged active; the current file dates from a 2020 migration (every family row was created from 2020-08-29 and attendance, schedule and billing rows start there) while student rows carry creation dates back to 1999; **5,654 student ids referenced by billing rows are absent from the live file** (214,642 billing-month rows, 1,444 of them carrying a payment), so the archive decision must say where those students live and how their money history imports; instructor joins go by `id_staff`, present on all but one attendance row, with the text name a fallback for three first-name collisions none of which is active twice; 46 `DUMMY` placeholder schedules, 9,582 instructor-break rows and 42,680 rows with no lesson type (FileMaker's `flag_hold`) are not enrollments and need a row-kind rule; 49 stored student first names are ALL CAPS without the special-needs flag and 138 primary-contact first names are ALL CAPS — the decode pass's actual list. The **archaeology** track (institutional artifacts) runs alongside without gating this phase.

**Gate:** the import runs from `BlueBuoy_FM` — confirmed live (Round 2 Q18) — and the `BlueBuoy_FM_2024` archive is accounted for per the archive decision, including the 5,654 students the live file's billing rows reference but no longer hold; household, student, enrollment, and flag counts reconcile against FileMaker; ten sampled households match field by field, including flags and notes; no deck-visible convention remains undecoded.
...
Enrollment, schedule editing, waitlist, closures, and search. Search is the phase's centre of gravity, not a feature within it: `saved_search` plus a criteria builder reaching every field staff currently search, seeded with the Saved Finds Q56 names. Plus make-up credit issuance and redemption — including conversion between denominations and sibling transfers, per [ADR-0003](../adr/0003-denominated-makeup-credit-ledger.md) — closure handling (scheduled vs incidental, bulk issuance by weekday or pool, the once-per-household skip-and-prorate waiver), held seats and instructor time blocks as two constructs, make-up offers of one or two options with short sequential holds, eligibility validation with override and per-class group bands, one-day repurposing of an empty group slot, the absence lookup as an instant action on the schedule board, the substitute-finding flow with its three constraints (instructor gender, level pairing, sibling proximity), and the deck-manager tools held out of the December slice.

The make-up ledger imports here by **replay**: every `Lesson_Out` row becomes an issued credit (rows flagged do-not-issue import as issued-then-voided), every make-up on `Lesson_Schedules` a consumption in the denomination `Lesson_MU_to_use` names, and the replayed balance is compared per student to the stored `MU_*_Total` — every difference is a reconciliation exception, with two known causes already named (an attendance script that edits counters directly; the do-not-issue flag that never suppressed the credit flag). The rehearsal against the 2026-09-14 export ([data profile](../discovery/data-profile-2026-09-14.md)) shows the stored counters cannot be the yardstick: FileMaker's own formula — flagged out-lessons minus flagged make-up schedules, a stored number re-evaluated only when a trigger fires — reproduces the stored value for just 14–31% of students with activity per denomination, and the ADR-0003 rule for 22–42%; the counters are stale, not the rows. Settled by [ADR-0005](../adr/0005-makeup-balance-reconciliation-at-import.md): the ledger is replayed from the rows, then one `reconciliation` event per student per denomination issues or consumes the difference to the stored counter, so no family's balance changes at go-live and the stale part is isolated into one reversible, reviewable event. Negative counters import as zero with the negative on the event and a mandatory review; the Unknown denomination (minus the adult-redemption count) is not imported and its 651 students join the adult review list. The office confirms or escalates reconciliations for students with an open enrollment or a lesson in the last twelve months and a non-zero difference; management corrects; everything else imports marked unreviewed and surfaces as a marker, never a block, when the family next books a make-up.

**Gate:** every make-up reconciliation in the review population (open enrollment or a lesson in the last twelve months, non-zero difference) is confirmed or corrected, the count of unreviewed reconciliations outside it is reported, and the total reconciliation liability in private-lesson equivalents is reported to ownership and accepted ([ADR-0005](../adr/0005-makeup-balance-reconciliation-at-import.md)); every search in the [parity enumeration](../discovery/search-parity-enumeration.md) — the scripted and structured searches the DDR encodes plus the daily finds and Saved Finds from Q55–Q58 — returns the same set as FileMaker on the same data, and deck staff run one full week of real scheduling in the new UI alongside the old, including one full make-up cycle.

### Billing

Constrained by ADR-0001: begins only after the December 2026 rollover has run in FileMaker, and never cuts over in a peak month. The billing run state machine, pricing, credits, proration, rate-lock price agreements, the first-responder discount, enrollment holds and the one-year hold clock, class packs consumed on attendance ([ADR-0004](../adr/0004-adult-class-packs.md) — legacy has nothing to replay and nothing to import either — the 2026-09-14 export shows `Adult_Credit` blank on every schedule row, so every adult enrollment lands on a review list with an unknown opening balance for the office to confirm; FileMaker's monthly Adult rate rows import like any other pre-created month), payment requests by hosted link so no staff member handles a card, the billing-exemption list with reasons, trade payments, the full transaction ledger, invoice PDFs, the Authorize.Net integration, and the monthly prepay-ending and tuition-vs-batch checks as exception views. Dry runs only; nothing charges a card until the shadow gate is green.
...
Freeze FileMaker writes, run the final import, verify counts, point the gateway at the new system, and keep FileMaker readable for a year alongside a permanent frozen archive that includes `BlueBuoy_FM_2024`, taken after the legacy card scrub so the archive holds no full card numbers. The card vault stays where it is, so no family is asked for a card.
