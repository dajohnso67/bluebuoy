...
*Substantially reduced after two rounds of staff questions. Listed explicitly so remaining gaps aren't mistaken for completeness.*

## Business decisions requiring an owner
- Is the sibling discount cap at the 4th slot still intended policy?
- What is the late-cancellation cutoff for forfeiting a make-up credit?
- Should make-up credits expire? (With confirmed conversion ratios, 24–28 banked credits represent a larger liability than a raw count suggests.)
- **How should Adult class no-shows be handled?** Staff flagged the current behaviour as a problem needing a policy answer.
- Is family or student the authoritative billing entity?
- How long should a make-up offer be held before cascading?
- Should the ALL-CAPS "difficult parent" convention be carried forward or replaced?

## Facts still to gather
- **Effective card processing rate** from Affinity24, and whether Authorize.Net Account Updater is enabled.
- **Whether current FileMaker backups have ever been restore-tested.**
- **Whether Humanity offers an API** (would allow read-only availability import — low priority, since guard-shift conflicts were confirmed as a non-issue).
- Purpose of legacy files `11-Wait`, `19`, `19 Time`, `20_BACKUP`.
- All data profiling listed in the technical appendix — requires a CSV export.

## Areas still thin
- **The QuickBooks / Excel institutional billing process** — described in detail but never seen. Screenshots would close this.
...
Group class structure, capacity and eligibility · make-up credit conversion ratios (bidirectional, confirmed) · sibling credit transfers · Adult class billing model · first-responder discount and stacking · monthly card volume and gross revenue · which FileMaker file is live (and why the archive exists) · the substitute-finding process · guard shift conflicts (non-issue) · **the complete reporting picture**.
...
3. **Credit card numbers are stored in plain text fields** in FileMaker. This is a live PCI exposure that ends when card storage moves to Accept.js/CIM tokenization.

---

*Compiled from DDR analysis, live system screenshots, and two rounds of interviews with Blue Buoy ownership and staff. Treat findings as verified where marked, and everything else as a starting point for verification.*
