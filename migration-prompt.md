...
| File | What it is | Read it before |
| --- | --- | --- |
| [`docs/migration/BlueBuoy_Complete_Context_Package.md`](./docs/migration/BlueBuoy_Complete_Context_Package.md) | The deepest domain document (v3, 2026-09-16; v1 and v2 live in git history), assembled from DDR analysis, live screenshots, ownership conversations, and three answered staff rounds. Technical appendix (verbatim billing calculations, verified join predicates, lesson-type codes, naming conventions, data landmines), Part A product requirements with staff answers folded in, Part B an alternative rollout plan, Part C its own questionnaire draft. Every claim carries a confidence label: **Confirmed / Reported / Inferred / Open**. | Designing any workflow, rule, or migration step; writing any code that touches legacy data |
| [`docs/migration/BlueBuoy_Export_Session_Findings.md`](./docs/migration/BlueBuoy_Export_Session_Findings.md) | The 13–14 September 2026 export-session addendum: how the live export was taken, corrections to the package (soft-delete flag, `Audits` scope, relationship labels, card storage), new business rules from ownership (held seats vs instructor blocks, override actions, trades, do-not-bill reasons, pool load, big-pool readiness, payment links) and the **Round 3** staff answers embedded in it. Cited as "Findings §n". Sanitised copy; the original is gitignored. | Anything touching cards, holds, closures, or the decode pass |
...
| [`docs/migration/BlueBuoy_Round2_Questions.docx.md`](./docs/migration/BlueBuoy_Round2_Questions.docx.md) | The second staff round as answered (2026-09-10): twenty questions in its own numbering, transcribed into `qa.md`. The committed copy has its eight screenshots removed (they show family names and contact details); the gitignored originals under `resources/artifacts/institutional/` and `resources/q-and-a-context/` keep them. | Reading a staff answer in the staff's own words, or citing "Round 2 Qn" |
| [`docs/migration/BlueBuoy_Web_App_PRD.md`](./docs/migration/BlueBuoy_Web_App_PRD.md) | The package's Part A extracted verbatim under a "React + PostgreSQL" title. The title is not the stack: architecture defers to `PLAN.md` and the table above. Read the package instead; this file exists so a reader handed only the PRD can be pointed back. | Nothing — cite the package section |
...
- **Four question-numbering schemes.** `qa.md` (current), `qa1-1.md` (offset −17 from Q24 on), the package's Part C (its own numbering — the shift-start PIN is Q87 there and Q83 in `qa.md`), and the Round 2 document (Round 2 Q1–Q20, transcribed into `qa.md` Q36, Q59–Q61, Q99–Q101 and Q103–Q108). A bare "Q41" is ambiguous; every citation carries its document. Round 3 (September 2026) has no numbering of its own: its answers sit inside the export-session findings and are cited by findings section ("Findings §3.1"); the package v3 Part D list renumbered again when four questions were inserted at 6–9.
...
3. **Reconciliation sweep 2** — done 2026-09-10 ([issue #10](https://github.com/dajohnso67/bluebuoy/issues/10)): the second staff round transcribed into `qa.md`, six more register rows flipped, the make-up credit ledger and the Adult class billing model sent to the map as design tickets.
4. **Then build**, phase by phase, gate by gate, per the reconciled `PLAN.md` — once the wayfinder map ([issue #1](https://github.com/dajohnso67/bluebuoy/issues/1)) has no decision blocking the phase's gate.
